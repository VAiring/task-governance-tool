...
### TG-M21.4, TG-M21.4A, TG-M21.4B, And TG-M21.4C Corrections
...
TG-M22.1 changes no behavior. TG-M21.4 corrects its inactive Receipt projection, while TG-M21.4B and TG-M21.4C repair the current-v17 recovery and stored-read baselines before TG-M22.2 advances v0.12.0 to schema v18 subject/ledger capture; TG-M22.3 retains v0.12.0 at schema v19. All retain 21 leaves/call bounds; old rows/cycles remain legacy, caller attestations stay `bound_attestation`, and JSON remains a non-authoritative one-way projection.

### Approved TG-M22 Sequence

All nine units, including the five M21 additions, are sequential Tier 2 work in lane `TG-M22-EVIDENCE-LEDGER`.
...
| TG-M22.2 / 30 | `tg_task_88bfe19eb6cffe2e` | accepted TG-M21.4C | Activate schema v18 authority snapshots, criteria, verification subjects, evidence references, deterministic manifests, and 1,000-character durable/read capacity while public Task add/edit remains 500. Reuse and extend the M21.4C validator; preserve strict M21.4B pre-selection set-fatal versus post-plan restore-fatal recovery and durable-prefix behavior. Preserve v17 Receipt/cycle bytes as subject-basis zero; fresh targets create basis one, Receipt input drops caller labels, and capture-v0 evidence writes fail closed. Preserve 21 leaves/call graph; add no bundle/JSON, analyzer, Runner, caller artifact list, raw content, or decision authority. Synchronize `AGENTS.md` retention, docs, Skill/reference, Viewer/package/migrations, split focused/full tests, record the exact compatibility package baseline, self-host retarget/reverification/review, exact-target Receipt, and two reviews. |
| TG-M21.5 / 40 | `tg_task_e7701fb907020905` | accepted TG-M22.2 | Raise only explicit public Task add/edit verification admission from 500 to 1,000 characters at schema v18; change or duplicate no shared stored validator, DDL, migration, backfill, other field limit, subject, leaf, or call. Preserve exact bytes, privacy, output caps, and compatibility with the complete recorded M22.2 package; require focused/full tests, exact-target Receipt, and two reviews. |
| TG-M22.3 / 50 | `tg_task_ae6f52c4f7b25549` | accepted TG-M21.5 | Activate schema v19 native bundles, criterion links/finding snapshots, projection generation, subject-based Evidence JSON and index-last publication, setup repair, and bounded maintenance. Consume the M22.2-stored/M21.5-public 1,000-character boundary and shared stored Task validator without changing either. Preserve exact INTEGER recovery metadata, pre-selection set-fatal versus post-plan restore-fatal classification, no reselection, durable completed-prefix reporting, semantic package schema examples, and observable TOCTOU tests without fixed internal call counts through schema-v19 setup/recovery. Native output contains the tool-owned subject and caller-attested result/coverage but no legacy caller label. Forbid import, custom output, service/network, narrative, analyzer, Runner, Viewer evidence UI, raw content, new leaf, or new LLM call; require synchronized contracts, migration/recovery/full-offline/exact-diff Receipt, and two reviews. |
| TG-M22.4 / 60 | `tg_task_0a90b4caf566a8fd` | accepted TG-M22.3 | Accept repaired M21.4B, M21.4C, M22.2, M21.5, and M22.3 across the exact v17 500/500, M22.2 v18 500-public/1,000-stored, and M21.5 v18 1,000/1,000 matrix. Include current-v17 privacy-invalid stored rows across every full/compact/Viewer loader; exact INTEGER recovery metadata; pre-selection set-fatal and post-link restore-fatal durable-prefix cases; shared validation through schemas 18/19; semantic package schema examples; and observable TOCTOU assertions without fixed call counts. Also cover unchanged legacy rows/cycles, safe old-package operation, capture-v0 rejection, complete lifecycle/evidence, Git artifacts, immutable bundles, DB/JSON equivalence, repair, privacy/size/no-write, command inventory, local consumer, package/release, full offline, and forward scenarios. Permit only bounded corrections; add no M23/M24 activation, Viewer evidence UI, command, unrelated refactor, or weakened test. Require exact-target Receipt and two reviews. |
