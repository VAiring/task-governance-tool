...
def preserved_business_projection(connection: sqlite3.Connection) -> dict:
    """Project pre-schema-18 durable rows that relocation must preserve."""
    ...
    for table, columns in excluded_columns.items():
        available = [
            str(row[1])
            for row in connection.execute(f'PRAGMA table_info("{table}")')
            if str(row[1]) not in columns
        ]
        projection[table] = tuple(
            sorted(
                (
                    tuple(row)
                    for row in connection.execute(
                        "SELECT "
                        + ", ".join(f'"{column}"' for column in available)
                        + f' FROM "{table}"'
                    ).fetchall()
                ),
                key=repr,
...
class M17ReleaseAcceptanceTests(unittest.TestCase):
    def test_realistic_pre_v9_legacy_layout_migrates_same_binding(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(migrated["data"]["schema_to"], 18)
            self.assertEqual(
