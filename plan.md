...
Decision baseline: v0.10.0 is published from exact commit `a9b80ce177a6dead10d51a070b76ff01f7af0294`; remote `main` and lightweight tag `v0.10.0` resolve to it, and GitHub Release `362617903` has prerelease visibility.
The M20 studies selected the accepted but inactive TG-M20S.3 decomposition design and the now-current TG-M21 Verification Receipt contract. Approved M21 activation/acceptance, accepted but inactive M21.4 verification-subject/capacity corrections, approved inactive M22/M23 evidence sequences, and revision-zero TG-M12.3 prerequisites are retained below.
The Task database, queried through the public CLI, solely owns live execution status and evidence.
The current unpublished local candidate identity is v0.11.0/schema v17/Viewer snapshot v4 with source compatibility v5-v17 and 21 public command leaves; it claims no tag, remote commit, or published Release.
...
### TG-M21.4 Through TG-M21.4D Corrections And TG-M22.1A Review Provenance
...
TG-M21.4B `tg_task_9b746fbe5fe4927f` reconciles current v17 recovery so only source-schema Task-verification capacity/privacy rejection may expose an older candidate; pre-selection structural faults remain set-fatal with no fallback, while post-plan TOCTOU faults remain restore-fatal with no reselection. Exact subject, compatibility, capacity, and recovery contracts live in the active specification/design.
TG-M21.4C `tg_task_efa90606fed8fba0` closes the current-v17 stored-Task read boundary before schema-v18 activation. One shared source-schema-aware row/batch validator must reject privacy, capacity, storage-type, enum, and cross-field-invalid state before every full, compact, Viewer, Review Packet, doctor, or lifecycle projection; errors remain sanitized, valid output stays compatible, and a failed Viewer refresh retains the last-good file. TG-M21.4D `tg_task_7051724dca3f1501` completes that boundary by validating every positive `current_contract_revision` against the latest immutable same-project/same-Task Contract revision through one bounded bulk relationship read. Dangling, foreign, nonlatest, and revision-zero-with-row states fail with the existing sanitized stored-state error before projection or write. Intentional empty-target/positive-generation invalidation and canonical stored-lane behavior remain unchanged; M22.2 must extend the corrected relation-aware boundary to schema v18.
TG-M22.1A `tg_task_0e1d93d81eb843ab` then freezes a closed conditional Review provenance contract after accepted TG-M21.4D and before schema-v18 activation: human review may declare model `not_applicable`, an LLM may declare Skill `not_used`, explicit v1 values remain caller-attested, and absent v17 provenance remains `legacy_unknown/legacy_migration` without changing original Receipt assurance. `unknown` and not-required remain distinct. It changes no v17 runtime or review gate; M22.2 owns storage/input/reference, M22.3 owns Bundle/JSON projection, M22.4 owns integrated acceptance, and M23 owns non-inferential reporting.

### TG-M22 Accepted Inactive Evidence Ledger Decision

TG-M22 freezes a canonical local Evidence Ledger, one immutable bundle per future native completion cycle, and a fixed one-way DB-to-JSON projection; exact assurance/producer, criterion, manifest, bundle, legacy, repair, privacy, and future-producer contracts live in the specification/design, while this plan owns only sequence and gates.

TG-M22.1 changes no behavior. TG-M21.4 corrects its inactive Receipt projection, while TG-M21.4B through TG-M21.4D repair the current-v17 recovery, stored-read, and Contract-pointer baselines before TG-M22.2 advances v0.12.0 to schema v18 subject/ledger capture; TG-M22.3 retains v0.12.0 at schema v19. All retain 21 leaves/call bounds; old rows/cycles remain legacy, caller attestations stay `bound_attestation`, and JSON remains a non-authoritative one-way projection.

### Approved TG-M22 Sequence

All eleven units, including the six M21 additions and TG-M22.1A, are sequential Tier 2 work in lane `TG-M22-EVIDENCE-LEDGER`.
Live status, targets, evidence, blockers, and completion commits remain solely in the Task database.
...
| TG-M21.4B / 19 | `tg_task_9b746fbe5fe4927f` | accepted TG-M21.4A | Reconcile recovery classification across docs, resolver/setup/backup, tests, and the M22.2 Contract. Skip only source-schema Task-verification capacity/privacy rejection; keep pre-selection corruption, foreign identity, lineage/metadata/structure set-fatal with no fallback and post-plan TOCTOU restore-fatal with no reselection. Enforce exact SQLite metadata storage classes before coercion, report the exact durable setup prefix after canonical linking, keep package schema examples current, and test observable revalidation order without brittle internal call counts. Require deterministic errors/no-write, focused/full/release checks, and two reviews. |
| TG-M21.4C / 20 | `tg_task_efa90606fed8fba0` | accepted TG-M21.4B | Add one shared source-schema-aware stored Task row/batch validator for current v17 and route every Task-loading and public projection path through it before allow-list or omission. Reject privacy, capacity, storage-type, enum, and cross-field-invalid rows with one sanitized fail-closed error and no rejected-byte leak; preserve valid outputs, bounded query behavior, no-write reads, and the last-good Viewer. Add no schema, command, leaf, normal-loop call, M22 behavior, or target-project write. Require shared fault fixtures, focused/full/package checks, exact-target Receipt, and two reviews. |
| TG-M21.4D / 24 | `tg_task_7051724dca3f1501` | accepted TG-M21.4C | Extend the shared current-v17 Task validator with one bounded bulk relationship read for `current_contract_revision`. Revision zero must have no Contract revision; a positive pointer must resolve to the latest same-project/same-Task row. Fail dangling, foreign, nonlatest, or revision-zero-with-row state before projection or write with the fixed sanitized error, no N+1, no schema/leaf/call change, and no broad cross-table audit. Preserve empty-target/positive-generation invalidation and canonical stored-lane behavior. Require focused/full/package checks, exact-target Receipt, and two reviews. |
| TG-M22.1A / 25 | `tg_task_0e1d93d81eb843ab` | accepted TG-M21.4D | Freeze the versioned Review provenance matrix and synchronize active docs plus downstream Task Contracts only. Distinguish reviewer class, conditional model/Skill declaration, lenses/profiles, context, methods, not-applicable, not-used, unknown, not-required, and legacy without proving identity, competence, independence, or truth. Add no runtime/schema/command/call/Viewer UI; require exact diff, current Receipt, and two reviews. |
| TG-M22.2 / 30 | `tg_task_88bfe19eb6cffe2e` | accepted TG-M22.1A | Activate schema v18 authority snapshots, criteria, verification subjects, evidence references, deterministic manifests, conditional Review provenance, and 1,000-character durable/read capacity while public Task add/edit remains 500. Extend the M21.4D relation-aware Task validation boundary and preserve strict M21.4B recovery. Preserve v17 bytes as explicit legacy; new Review receipts distinguish human/no-model, LLM/no-Skill, declared, tool, hybrid, and unknown states through the existing receipt input/read/reference paths without changing reviewer-key, PASS, Tier, or completion gates. Preserve 21 leaves/call graph; add no bundle/JSON, analyzer, Runner, raw content, decision authority, new command/call, or Viewer evidence UI. Synchronize docs, Skill/reference, package/migrations, compatibility baseline, exact-target Receipt, and two reviews. |
| TG-M21.5 / 40 | `tg_task_e7701fb907020905` | accepted TG-M22.2 | Raise only explicit public Task add/edit verification admission from 500 to 1,000 characters at schema v18; change or duplicate no shared validator, DDL, migration, backfill, field limit, subject, Review provenance input/read/reference/gate, leaf, or call. Preserve exact bytes, privacy, output caps, and compatibility with the complete recorded M22.2 package; require focused/full tests, exact-target Receipt, and two reviews. |
| TG-M22.3 / 50 | `tg_task_ae6f52c4f7b25549` | accepted TG-M21.5 | Activate schema v19 native bundles, criterion/finding links, projection generation, Evidence JSON/index-last publication, setup repair, and bounded maintenance. Consume unchanged M22.2 provenance/capacity and preserve recovery/package/TOCTOU contracts. Bundle/JSON preserve exact human/no-model, LLM/no-Skill, declared, tool, hybrid, unknown, and not-required states. Explicit v1 provenance is `bound_attestation/trusted_caller/1`; absent v17 provenance is `legacy_unknown/legacy_migration` and original Receipt assurance is unchanged. Forbid import, service/network, narrative, analyzer, Runner, Viewer evidence UI, raw content, new leaf/call; require migration/recovery/full-offline/exact-diff Receipt and two reviews. |
| TG-M22.4 / 60 | `tg_task_0a90b4caf566a8fd` | accepted TG-M22.3 | Accept repaired baselines, including the M21.4D Contract-pointer relationship boundary, and M22 across capacity, shared validation, recovery, package, and TOCTOU matrices. Cover legacy safety, lifecycle/evidence, artifacts, immutable bundles, DB/JSON equivalence, repair/privacy/size/no-write, all conditional Review states, repeated profiles, and explicit-v1 versus absent-legacy assurance. Repetition is descriptive only; no inference, assurance upgrade, quality score, or gate change. Permit bounded corrections only; add no M23/M24 activation, Viewer evidence UI, command, refactor, or weakened test. Require exact-target Receipt and two reviews. |

The sequence may advance only in that order. M22.2, M21.5, and M22.3 each land a coherent supported intermediate state; the schema-v18 revisions remain forward-data compatible without a new marker, and none may expose behavior whose Viewer compatibility, package inventory, or current formal contracts are unsynchronized.
M22 grants no publish, tag, Release, push, external-service, or target-project mutation authority.

### Approved TG-M23 Derived Evidence Sequence

TG-M23 is approved but inactive sequential Tier 2 work in `TG-M23-DERIVED-EVIDENCE`. It consumes sealed M22 bundles without direct SQLite access, adds no normal-loop call or gate authority, keeps offline rendering sufficient, and requires separate current authority for any live external-model execution.

| Unit/order | Task | Dependency | Purpose, bounded scope, permission, and completion gate |
|---|---|---|---|
| TG-M23.1 / 10 | `tg_task_722ac8a308a23d1c` | accepted TG-M22.4 | Freeze the design-only outbox, bounded packet, worker/validator, immutable report, exact citation, conditional Review-provenance rendering, privacy, retry, and offline/optional-inference contracts. Activate no runtime/schema/CLI/Skill/network behavior; require exact diff, documentation checks, and two reviews. |
| TG-M23.2 / 20 | `tg_task_d5511d2ca7db93dc` | accepted TG-M23.1 | Activate deterministic outbox/packet/worker/report storage and offline renderer, with an optional separately authorized model adapter. Read bundles rather than SQLite; preserve facts, assurance, and gates; add no task-loop call or Task mutation; require privacy/package checks, exact diff, and two reviews. |
| TG-M23.3 / 30 | `tg_task_0ada32d2b4f9759d` | accepted TG-M23.2 | Accept queue/restart/failure, exact citations, immutable reports, conditional and legacy Review provenance, privacy, and non-blocking behavior. Offline/mocked acceptance is sufficient when live execution is unauthorized; when separately authorized the exact live smoke must pass. Permit bounded fixes only; require full offline checks, exact diff, and two reviews. |
