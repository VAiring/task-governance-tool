...
class DbInitTests(unittest.TestCase):
    def test_db_init_creates_temp_database_with_json_payload(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payload["data"]["migrations_applied"], [1, 2, 3, 4, 5, 6, 7, 8])
    ...
    def test_db_init_rejects_incomplete_migration_history_without_false_success(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(versions, [1, 2, 3, 5, 6, 7, 8])
    ...
    def test_db_init_migrates_schema_v1_database_through_paused_state(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            payload = json.loads(result.stdout)
            self.assertFalse(payload["data"]["created"])
            self.assertEqual(payload["data"]["migrations_applied"], [2, 3, 4, 5, 6, 7, 8])
            self.assertEqual(payload["data"]["schema_version"], 8)
            with closing(sqlite3.connect(db)) as connection:
                connection.row_factory = sqlite3.Row
    ...
    def test_db_init_warns_when_inconsistent_legacy_evidence_is_preserved(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            payload = json.loads(result.stdout)
            self.assertEqual(payload["data"]["migrations_applied"], [4, 5, 6, 7, 8])
    ...
    def test_git_snapshot_migration_preserves_v5_review_evidence_and_is_idempotent(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payload["data"]["migrations_applied"], [6, 7, 8])
            self.assertEqual(payload["data"]["schema_version"], 8)
