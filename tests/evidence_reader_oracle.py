"""Independent test-only reader for sealed Evidence JSON.
...
without importing the Analyzer runtime or producer validator.
"""
...
def _validate_source_basis(value: object, *, source_kind: str) -> dict[str, Any]:
    """Retain the selected entry with its exact index and schema versions."""

    if source_kind not in {"native_bundle", "legacy_index_entry"}:
        _invalid()
    basis = _mapping(
        value,
        (
    ...
    entry = _validate_index_entry(
        basis["entry"],
        index_format_version=basis["index_format_version"],
    )
    expected_state = (
        "native" if source_kind == "native_bundle" else "legacy_unknown"
    )
...
class ValidatedEvidenceSource:
    ...
    def __init__(
        ...
        basis = _validate_source_basis(source_basis, source_kind=source_kind)
        entry = basis["entry"]
        expected_bundle_format_version = entry.get("bundle_format_version", 1)
        ...
        if source_kind == "legacy_index_entry":
            ...
            if (
                ...
                or bundle_format_version != expected_bundle_format_version
            ...
            payload = _validate_bundle_payload(
                ...
                maximum_source_schema_version=basis["source_schema_version"],
            )
            domain = (
                BUNDLE_DOMAIN
                if bundle_format_version == 1
...
def revalidate_validated_source(value: object) -> ValidatedEvidenceSource:
    """Purely revalidate one immutable Evidence source."""
...
def read_evidence_index(
    ...
    *,
    ...
    except (StatePathError, EvidenceCodecError, OSError) as exc:
        raise EvidenceConsumerError() from exc
    payload = envelope["payload"]
    project_id = payload["project_id"]
...
def validate_evidence_source(
    ...
    basis = {
        "index_format_version": stable_index.format_version,
        "source_schema_version": stable_index.source_schema_version,
