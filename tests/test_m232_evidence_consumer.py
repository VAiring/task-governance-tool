...
class EvidenceConsumerTests(unittest.TestCase):
    ...
    def test_validated_index_owns_canonical_bytes_and_rejects_spoofs(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            source = validate_evidence_source(index, legacy)
            ...
            with self.assertRaises(EvidenceConsumerError):
                revalidate_descriptor_source(spoof, descriptor)
            self.assertEqual(tree_snapshot(evidence), before)
    ...
    def test_descriptor_revalidation_preserves_original_basis_across_index_drift(self):
        with tempfile.TemporaryDirectory() as temporary:
            evidence = write_evidence_tree(Path(temporary))
            original_index = read_evidence_index(evidence)
            native = next(
                entry
                for entry in original_index.entries
                if entry["bundle_state"] == "native"
            )
            original_source = validate_evidence_source(original_index, native)
            descriptor = build_descriptor(
                source_kind=original_source.source_kind,
                source_basis=original_source.source_basis,
                recipe=default_recipe(),
            )
            ...
            self._rewrite_index(
                ...
                lambda payload: payload.update(
                    {"projection_generation": payload["projection_generation"] + 1}
                ),
            )
            current_index = read_evidence_index(evidence)
            before = tree_snapshot(evidence)
            replay_source = revalidate_descriptor_source(current_index, descriptor)
            self.assertEqual(replay_source.source_basis, descriptor["source_basis"])
            self.assertEqual(replay_source.source, original_source.source)
            packet = build_analysis_packet(descriptor, replay_source)
            self.assertEqual(packet.value["source_basis"], descriptor["source_basis"])
            self.assertEqual(tree_snapshot(evidence), before)

            altered_descriptor = deepcopy(descriptor)
            altered_descriptor["source_basis"]["projection_generation"] += 1
            with self.assertRaises(EvidenceConsumerError):
                revalidate_descriptor_source(current_index, altered_descriptor)
            ...
            self._rewrite_index(
                ...
                lambda payload: next(
                    ...
                    if entry["bundle_state"] == "native"
                ).update({"file_digest": "sha256:" + ("0" * 64)}),
            )
            mismatched_index = read_evidence_index(evidence)
            before = tree_snapshot(evidence)
            with self.assertRaises(EvidenceConsumerError):
                revalidate_descriptor_source(mismatched_index, descriptor)
            self.assertEqual(tree_snapshot(evidence), before)
